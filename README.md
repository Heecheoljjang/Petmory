...
- Value에 바로 접근하려고 Relay를 사용했는데 괜찮은지
    - 만약 다른 경우에는 try로 접근해도 괜찮은지 -> 자주 쓰는지
- 바버튼도 rx로 설정해주는지
    - 만약 menu의 UIAction같은 경우엔 children으로 접근하면 되는지
    - 만약 바버튼도 하는 경우엔 바버튼 프로퍼티를 뷰컨트롤러에서 따로 전역변수로 설정해야하는지, 만약 지역변수로 메서드안에서 선언된 경우엔 메서드 실행 순서가 중요한지
- 뷰모델에 있는 변수들을 전부 리액티브타입?으로 설정해야하는지
- delegateFlowLayout같은건 기존처럼 프로토콜 채택해서 해야하는지
- 두 개의 파라미터와 셀이 관련있는 경우엔 바인드를 두 번해줘야한느지
    - 이떄 생기는 오류가 delegate랑 datasource를 nil로 해서 해결할 수 있다는데 맞는지
- 릴레이로 선언됐을땐 테이블뷰 설정할때도 그대로 drive로 하면되는지
    - UI바꾸는 부분이니 맞는 것 같긴함
- 두 번하는 경우엔 두 뷰모델의 프로퍼티가 count가 같아야하는지 -> 이걸로 셀의 수가 결정되니까
    - 보통은 같긴할듯 row로 접근하니
