...
반려동물과의 소중한 하루를 기록해보세요
일기뿐만 아니라 일정도 등록할 수 있습니다 :)

<img width="600" alt="Untitled" src="https://user-images.githubusercontent.com/77842318/228937577-385e24d8-e1f0-46d1-9ca7-1e30c45bc458.png">

- 반려동물과의 하루를 사진과 함께 작성할 수 있어요
- 작성한 일기들을 월별로 확인할 수 있어요
- 날짜를 설정하여 과거나 미래의 일기도 남길 수 있어요
- 함께한 반려동물들을 선택하여 반려동물별로 일기를 확인할 수 있어요
- 작성한 모든 일기들을 한 번에 모아볼 수 있어요
- 반려동물과의 일정을 잊지 않도록 캘린더에 등록할 수 있어요
- 소중한 일기를 백업하여 안전하게 보관할 

AppStore: [펫모리](https://apps.apple.com/kr/app/%ED%8E%AB%EB%AA%A8%EB%A6%AC-petmory/id6443397065)
...
### 1.1.0
- 등록된 반려동물이 한 마리인 경우, 작성화면에서 자동으로 선택되도록 바꿔주었음.
- 복구 시 펫모리의 백업 파일인지 확인하는 로직 추가
  - 확인할 수 있는 txt파일을 다른 데이터들과 함께 압축 후, 복구했을 때 해당 txt파일이 있는지 확인하도록 하였음.
- 버전 정보 추가
- DatePicker에서 취소버튼 누르면 이전의 마지막 스크롤 포지션이 남아있는 오류 수정.
  - DatePicker를 띄우는 동시에 가지고 있던 현재 값을 date로 설정
- 복구 후 알
- UI 수정
  - 툴바의 tintColor 변경
  - 'done -> 완료'로 수정
  - 타이틀의 날짜를 누를 수 있는지 모르는 경우가 있어, 조그만 화살표 추가
  - 테이블뷰 separator 추가
  - 디테일 화면 텍스트 잘리는 것 수정
  
 ### 1.2.0
 - 앱 버전 보이는 위치 수정
 - 백업 파일 확인하는 과정에서 압출을 풀면 도큐먼트 폴더 내에 필요없는 파일들이 쌓이는 상태여서, 임시 폴더내에 압축 해제 후 삭제하는 방식으로 변경
 - 반려동물의 이름이 수정되는 경우, 기록 데이터들도 전부 변경해줌.
 - 반려동물의 이름 중복체크 로직 추가
 - g,y,p같이 내려온 알파벳 잘리는 문제 해결
 - Analytics와 Crashlytics 적용
 - 접근 제어자 추가
 - 일정에서 제목이 길면 확인할 수 없는 오류 수정
 - 하드코딩 열거형으로 정리
 ...
 ### 1.2.2
 - 일정 등록할 때, 제목 수정사항이 초기화되는 오류를 수정
 - MVVM, RxSwift 일부 적용
...
- 설정화면 테이블뷰를 컬렉션뷰의 컴포지셔널 레이아웃으로 변경
    - 이전과 좀 달라져서 다시 돌려야할지도
...
- 반려동물 등록화면
    - 조건체크 로직 개선
        - 기존의 if문 중첩으로 인한 뎁스를 줄임.
        - 여러 개의 if문과 return을 이용
- 설정화면
    - 일단 업데이트를 올리기 위해 다시 테이블뷰로 바꿈
...
- bind로 테이블뷰에 접근할때 element를 이용하면되는걸 row와 뷰모델의 프로퍼티를 이용했음
- Main화면에서 하나의 컬렉션뷰 셀 안에 뷰모델의 두 프로퍼티를 쓰는 경우가 있었음. 이때 bind를 두번했는데 이러면 오류가 발생함
    - delegate와 dataSource를 nil로 바꿔주라는데 내일 더 찾아봐야할듯
...
Main화면에서 하나의 컬렉션뷰 셀 안의 뷰모델의 두 프로퍼티를 쓰는 경우가 있었음. 

- 처음 방법
    - 각 프로퍼티마다 bind를 해주어 총 두번의 bind를 했음.
        - 이미 대리자가 있다는 오류가 발생해서 다시 바인드해주기전에 nil을 하라는 오류가 뜸
        - 해봤지만 해결못했음
- 실패한 방법들
    - tempList와 CountList 프로퍼티가 각각 쓰이므로 combineLatest로 묶어서 해봄
        - 튜플형태는 시퀀스 프로토콜을 채택하지 않기 때문에 컬렉션뷰의 셀과 bind될 수 없었음
    - zip을 이용해서 ZipSequence형태로 묶어봄
        - 마찬가지로 같은 오류가 떴음
- 성공한 방법
    - 튜플이 안된다면 튜플을 원소로 갖는 배열을 사용함
        - 가능한 이유는 tempList와 CountList의 원소 수가 같기 때문
        - 그래서 뷰모델에서 튜플을 원소로 갖는 배열을 타입으로 갖는 BehaviorRelay타입 변수를 선언해서 메서드를 통해 값은 accept해줌
        - 이 변수와 컬렉션뷰를 bind하여 해결
            - countList와 tempList의 값을 넣는 과정에서 원소 개수가 맞지 않는 시점이 있었음.
            - 그래서 temp와count를 갖는 새로운 변수에 값을 넣는 메서드를 countList를 다 만든 이후에 실행하도록했음
            - countList는 tempList가 완성된 뒤에 값이 들어가므로 countList가 완성된 후에 메서드를 실행하면 tempList와 countList의 원소 수는 차이가 없으므로 오류가 발생하지 않음.
...
- RxDataSource
    - 테이블뷰의 헤더뷰가 커스텀한 형태일때는 사용할 수가 없었음.
    - 다른 부분이라도 적용해보려했지만 실패해서 다시 되돌림
    - 컬렉션뷰는 가능한데 테이블뷰는 왜 안되는지 모르겠음.
...
- AllMemory 화면
    - RxDataSource없애고 나머지에 대해서 Rx적용
    
#### 이슈

- 화면전환을 하면서 데이터를 넘겨줄 때 전환할vc.viewModel.프로퍼티.accept 같은 느낌으로 값을 넣어주는 코드가 있었음.
근데 이전 화면에서 다음 화면의 viewModel까지 굳이 접근해야되나하는 의문이 들었음. 
근데 뷰컨트롤러에 값을 받은 프로퍼티를 만들면 Input Output으로 정리할 수가 없기때문에 별로라는 생각이 들었음.
뷰모델을 추가적으로 만들거나 같은 뷰모델을 쓰게 해서 뎁스를 줄일 순 있는데 오버헤드가 클 것 같고, 같은 뷰모델을 쓰기에는 내용에도 차이가 있어 가독성에 문제가 생김.

- 인풋아웃풋이 너무 헷갈림
    - 코드를 많이 찾아봤는데 어느정도까지는 비슷해도 조금씩 차이가 있었음. drive나 bind부분에는 UI적인 핸들링이 들어가는 부분이 있을 수도 있어서 반드시 뷰컨트롤러에 있어야 할 것 같음. 그리고 뷰모델의 프로퍼티의 값을 가져와서 사용하는 것은 아웃풋이고, 실질적인 인터랙션 부분이 인풋이라고 생각했었음.
    근데 다시 생각해보면, 뷰모델의 프로퍼티 값을 가져와서 타입을 바꾸는 경우가 있으므로 이 경우엔 뷰모델의 프로퍼티가 인풋으로 들어갈 것 같음!!!
    작성해보고 여쭤보자.
